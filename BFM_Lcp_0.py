...
LEARNING_RATE = 1e-2#1e-2  # larger values converge faster to less accurate results
...
class BFM_Lcp_0:
    ...
    def gradient_descent(self, sess, model):
        ...
        newimg = (tf.tanh(modifier + simg) / 2) * self.norm_to_01(canchange) + (1 - self.norm_to_01(canchange)) * original#self.change_2(canchange)
        Initnewimg = newimg
        ...
        # sum up the losses
        Initloss_bin = tf.reduce_max(self.L_0loss(self.norm_to_01(canchange)))  #
        # Initloss_bin =tf.reduce_sum(    tf.square(canchange)   , (1, 2, 3))
        Initloss_mod = tf.reduce_max(self.L_0loss(    (Initnewimg - tf.tanh(timg) / 2.)*(tf.cast((Initnewimg - tf.tanh(timg) / 2.)>(0.9/255.),dtype=tf.float32))))#
        Initloss_mod = tf.reduce_max(self.L_0loss((tf.tanh(modifier + simg) / 2 )- tf.tanh(timg) / 2) / 10.)  # tf.reduce_max(tf.square(tf.tanh(modifier + simg) / 2 - tf.tanh(timg) / 2), (1, 2, 3))#
        # loss3 = self.change(    newimg - tf.tanh(timg) / 2. )
        # Initloss4 = self.change(    (Initnewimg - tf.tanh(timg) / 2.)*(tf.cast((Initnewimg - tf.tanh(timg) / 2.)<(0.9/255.),dtype=tf.float32)) )#
        Initloss = 10 * Initloss1 + 0.1* Initloss_bin * Initloss_mod  #
        ...
        def doit(oimgs, labs, starts):
            ...
            for step in range(self.MAX_ITERATIONS):
                    ...
                    _, works, ploss, qloss, Initloss_b, Initloss_m = sess.run(
                        [Inittrain, Initloss1, Initoutput, tlab, Initloss_bin, Initloss_mod], feed_dict=feed_dict)
                    # print("works",works)
                    # print("Initloss_b",Initloss_b)
                    # print("Initloss_m",Initloss_m)
                    if self.TARGETED:
                        if np.argmax(ploss) == np.argmax(np.squeeze(qloss)):
                            nimg = sess.run((Initnewimg), feed_dict=feed_dict)
                            if Initloss_b < old_Initloss_b:
                                old_nimg = nimg
                                old_Initloss_b = Initloss_b
                    else:
                        if np.argmax(ploss) != np.argmax(np.squeeze(qloss)):
                            nimg = sess.run((Initnewimg), feed_dict=feed_dict)
