...
class CSattack():
  ...
  def perturb(self, x_nat, y_nat,fake,overall_points, sess,targeted = True):
    adv = np.copy(x_nat)
    ...
    corr_pred = sess.run(self.model.correct_prediction, {self.model.x_input: x_nat-0.5, self.model.y_input: y_nat})
    bs = self.shape_img[0]*self.shape_img[1]
    for c in range(x_nat.shape[0]):
      if not targeted:
          flag = corr_pred[c]
      else:
          flag = not corr_pred[c]
      if flag:
        ...
        for counter in range(self.n_corners):
          logit_2[counter*bs:(counter+1)*bs], pred = sess.run([self.model.y, self.model.correct_prediction], feed_dict={self.model.x_input: batch_x[counter*bs:(counter+1)*bs]-0.5, self.model.y_input: np.tile(batch_y,(bs))})
          if not targeted:
            ...
            if pred.any() and not found:
              ind_adv = np.where(pred.astype(int)==1)
              adv[c] = batch_x[counter*bs + ind_adv[0][0]]
              found = True
              print('Point {} - adversarial example found changing 1 pixel'.format(c))
          ...
          if not found:
             for c2 in range(self.n_classes):
               if not found:
                 ...
                 pred = sess.run(self.model.correct_prediction, feed_dict={self.model.x_input: batch_x-0.5, self.model.y_input: np.tile(batch_y,(batch_x.shape[0]))})
                 ...
                 if not targeted:
                   ...
                   if np.sum(pred.astype(np.int32))>1 and not found:
                     ind_adv = np.where(pred.astype(int)==1)
                     adv[c] = batch_x[ind_adv[0][0]]
                     found = True
                     print('Point {} - adversarial example found changing {} pixels'.format(c, np.sum(np.amax(np.abs(adv[c] - x_nat[c]) > 1e-10, axis=-1), axis=(0,1))))
    ...
    overall_points += pixels_changed
    #print('attack successful: ', fl_success)
    #print('attack successful: {:.2f}%'.format((1.0 - np.mean(fl_success))*100.0))
    corr_pred = sess.run(self.model.correct_prediction, {self.model.x_input: adv-0.5, self.model.y_input: y_nat})
    ...
    return adv, pixels_changed, fl_success, fake,overall_points