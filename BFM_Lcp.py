...
class BFM_Lcp:
    ...
    def gradient_descent(self, sess, model):
        ...
        Initloss_mod = Initloss_smod#tf.reduce_mean(Initloss_smod)  # tf.reduce_sum(tf.square(tf.tanh(modifier + simg) / 2 - tf.tanh(timg) / 2), (1, 2, 3))#

        Initloss2 = self.change(    (Initnewimg - tf.tanh(timg) / 2.)*(tf.cast((Initnewimg - tf.tanh(timg) / 2.)<(0.9/255.),dtype=tf.float32)) )#
        Initloss =10.*Initloss1 + 0.5*Initloss2
